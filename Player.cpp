...
void Player::Update() {
	...
	// 5.壁に接触してる場合の処理
	HitsTheWall(collisionMapInfo);
	
    // 6.設置状態の切り替え
	HitsTheLanding(collisionMapInfo);
...
void Player::HitsTheLanding(const CollisionMapInfo& info) {
	...
	//設置状態の処理
		...
		onGround_ = false;
			...
			// 左下点の当たり判定
			MapChipField::IndexSet indexSet;
			indexSet = mapChipField_->GetMapChipIndexSetByPosition(positionNew[kLeftBottom] + Vector3(0.0f, -kAdjustmentSpace, 0.0f));
			mapChipType = mapChipField_->GetMapChipTypeByIndex(indexSet.xIndex, indexSet.yIndex);
			if (mapChipType == MapChipType::kBlock) {
				hit = true;
			}
			// 右下点の当たり判定
			indexSet = mapChipField_->GetMapChipIndexSetByPosition(positionNew[kRightBottom] + Vector3(0.0f, -kAdjustmentSpace, 0.0f));
			mapChipType = mapChipField_->GetMapChipTypeByIndex(indexSet.xIndex, indexSet.yIndex);
			if (mapChipType == MapChipType::kBlock) {
				hit = true;
			}
